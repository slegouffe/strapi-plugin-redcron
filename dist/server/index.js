...
const Redlock__default = /* @__PURE__ */ _interopDefault(Redlock);
const bootstrap = ({ strapi }) => {
  ...
  if (strapi.config.get("plugin::redcron-v5").debug) {
    debug__default.default.enable("strapi:plugin:redcron-v5");
    debug__default.default("strapi:plugin:redcron-v5")("\nredcron config:", strapi.config.get("plugin::redcron-v5"));
  }
};
  ...
  strapi.cron.add = (tasks) => {
    const generateRedlockFunction = (originalFunction, name) => {
      return async (...args) => {
        ...
        const redlock = new Redlock__default.default(connections, config2.redlockConfig);
        let lock;
        try {
          lock = await redlock.acquire([name], config2.lockTTL);
          debug__default.default(`Job ${name} acquired lock`);
          await originalFunction(...args);
        } catch (e) {
          debug__default.default(`Job ${name} failed to acquire lock`);
        } finally {
          ...
          debug__default.default(`Job ${name} waiting ${lockDelay}ms before releasing lock`);
          await new Promise((resolve) => setTimeout(resolve, lockDelay));
          if (lock) {
            debug__default.default(`Job ${name} releasing lock`);
            try {
              await lock.release();
            } catch (e) {
              debug__default.default(`Job ${name} failed to release lock ${e}`);
            }
          }
        }
      };
